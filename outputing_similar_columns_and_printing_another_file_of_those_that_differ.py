...
# Filter by substring or regex (e.g., "android")
if single_filter_regex:
    mask_filter = series_full.str.contains(single_filter_value, case=False, na=False, regex=True)
else:
    mask_filter = series_full.str.contains(single_filter_value, case=False, na=False, regex=False)

# Matched (contain filter) and non-matched (do not contain filter)
df_masked = df[mask_filter].copy()
matches = df_masked.copy()
non_matches = df[~mask_filter].copy()
...
if output_columns:
    ...
    out_nonmatches = non_matches[output_columns].copy()
else:
    out_df = matches.copy()
    out_nonmatches = non_matches.copy()

# --- SAVE RESULTS ---
output_file = f"matched_rows_containing_{single_filter_value}.xlsx"
out_df.to_excel(output_file, index=False, engine="openpyxl")

output_file_non = f"non_matched_rows_not_containing_{single_filter_value}.xlsx"
out_nonmatches.to_excel(output_file_non, index=False, engine="openpyxl")

print(f"Done! {len(out_df)} matched rows saved to: {output_file}")
print(f"Done! {len(out_nonmatches)} non-matched rows saved to: {output_file_non}")
